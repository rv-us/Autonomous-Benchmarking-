...
class PicarXAdvancedAgent:
    def __init__(self, api_key: str = None):
        """Initialize the advanced Picar-X agent with OpenAI API key."""
        ...
        self.agent = Agent(
            ...
            tools=[
                ...
                self.execute_plan_step_tool,
                self.get_task_status_tool
            ],
            memory=self.memory
        )
    ...
    def execute_plan_step_tool(self, step_number: Optional[int] = None) -> str:
        ...
        try:
            ...
            if step_number > len(self.task_plan):
                return "All plan steps completed!"
            
            step = self.task_plan[step_number - 1]
            self.current_step = step_number
            
            # Execute the step based on its content
            if "picture" in step.lower():
                result = self.capture_image_tool()
                self.task_history.append(f"Step {step_number}: {result}")
                return f"Executed step {step_number}: {step}\nResult: {result}"
            elif "distance" in step.lower():
                result = self.get_ultrasound_tool()
                self.task_history.append(f"Step {step_number}: {result}")
                return f"Executed step {step_number}: {step}\nResult: {result}"
            elif "look around" in step.lower():
                # Pan camera to look around
                self.set_cam_pan_servo_tool(-30)
                time.sleep(1)
                self.set_cam_pan_servo_tool(30)
                time.sleep(1)
                self.set_cam_pan_servo_tool(0)
    ...
    def get_task_status_tool(self) -> str:
        ...
        try:
            ...
            status = f"Current Task: {self.current_task}\n"
            status += f"Progress: {self.current_step}/{len(self.task_plan)} steps completed\n"
            status += f"Current Step: {self.task_plan[self.current_step - 1] if self.current_step > 0 else 'Not started'}\n"
            status += f"History: {len(self.task_history)} actions taken"
            
            return status
        except Exception as e:
            return f"Error getting task status: {str(e)}"
    
    def execute_long_form_task(self, task_description: str) -> str:
        """Execute a long-form task with planning and iteration."""
        try:
            # Create a plan
            plan_result = Runner.run_sync(self.agent, f"Create a plan for: {task_description}")
            print(f"Plan created: {plan_result.final_output}")
            
            # Execute the plan step by step
            while self.current_step < len(self.task_plan):
                step_result = Runner.run_sync(self.agent, f"Execute the next step in the plan")
                print(f"Step {self.current_step + 1}: {step_result.final_output}")
                ...
                if "obstacle" in step_result.final_output.lower() or "blocked" in step_result.final_output.lower():
                    adapt_result = Runner.run_sync(self.agent, "The path is blocked. Adapt the plan to find an alternative route.")
                    print(f"Plan adapted: {adapt_result.final_output}")
            
            # Get final status
            status_result = Runner.run_sync(self.agent, "Get the final task status")
    ...
    try:
        while True:
            ...
            if user_input.lower() == 'quit':
                ...
                result = agent.execute_long_form_task(user_input)
